...
@section('content')
    ...
    @if (session()->has('delete_invoice_temporary'))
        <script>
            window.onload = function() {
                notif({
                    msg: "تم حذف الفاتورة مؤقتا",
                    type: "success"
                })
            }
        </script>
                        ...
                        <table id="example1" class="table key-buttons text-md-nowrap">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">كمية الضريبة</th>
                            ...
                            <tbody>
                                @foreach ($invoices as $invoice)
                                    <tr>
                                        ...
                                        <td><a
                                                href="{{ url('InvoicesDeltails') }}/{{ $invoice->id }}">{{ $invoice->section->section_name }}</a>
                                        </td>
                                        ...
                                        <td>
                                            @if ($invoice->Value_Status == 1)
                                                <span style="color: rgb(10, 153, 10)">
                                                    {{ $invoice->Status }}
                                                </span>
    ...
    <div class="modal fade" id="transfer_invoice" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ url('Archive/update') }}" method="post">
                    {{ method_field('patch') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <p class="text-center">
                        <h6 style="color:green"> هل انت متاكد من عملية إلغاء الأرشفة ؟</h6>
                        </p>
                        <input type="hidden" name="id_invoice" id="id_invoice" value="">
                    </div>
                    <div class="modal-footer">
...
@section('js')
    ...
    <script>
        $('#transfer_invoice').on('shown.bs.modal', function(event) {
            var button = $(event.relatedTarget);
            var id_invoice = button.data('id_invoice');
            var modal = $(this);
